...
async def get_document_status(document_id: str = Path(..., description="Document ID")):
    ...
    try:
        ...
        artifacts = {
            ...
            is not None,
            "graph": graph_json_path(document_id).exists()
            or (
                settings.allow_legacy_pickle_loading
                and graph_pickle_path(document_id).exists()
            ),
        ...
        response = DocumentStatusResponse(
            ...
            has_active_job=latest_job is not None
        ...
        if latest_job and latest_job.status not in [
            ...
            if latest_job.started_at and latest_job.progress_percent > 0:
                ...
                response.estimated_completion = str(
...
async def cancel_job(job_id: str = Path(..., description="Job ID")):
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=400, detail="Job not found or already in terminal state"
            )
...
async def recover_document(
    document_id: str = Path(..., description="Document ID"), skip_completed: bool = True
):
    """
