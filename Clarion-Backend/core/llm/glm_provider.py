"""
GLM Provider - Implementation for ChatGLM models.
"""
...
class GLMProvider(BaseLLMProvider):
    ...
    Supports both ChatGLM API and local deployment via vLLM, Ollama,
    or other OpenAI-compatible servers.
    ...
    MODEL_CONFIGS = {
        "chatglm3-6b": {
            "context_window": 8192,
            "cost_input": 0.0,  # Local deployment = no API cost
            "cost_output": 0.0,
            "supports_json": True
        },
        "chatglm3-6b-32k": {
    ...
    def _initialize_client(self) -> None:
        """Initialize httpx async client for GLM API."""
        # API key might not be required for local deployments
        headers = {
    ...
    async def generate(
        ...
        **kwargs
        ...
        """
        Generate text using GLM chat completions API.
        ...
        try:
            ...
            raise LLMProviderError(f"GLM API error: {e.response.status_code}") from e
...
Respond with JSON only. Do not wrap in markdown code blocks."""
    ...
    def get_model_info(self) -> ModelInfo:
        """Get information about configured GLM model."""
        model = self.model or self.DEFAULT_MODEL
        ...
        return ModelInfo(
            ...
            supports_functions=False,  # May vary by deployment
    ...
    def validate_config(self) -> tuple[bool, Optional[str]]:
        ...
        GLM allows empty API key for local deployments.
        """
        if not self.model:
