...
class JobRepository:
    ...
    def add_artifact(self, job_id: str, artifact_key: str, artifact: ArtifactInfo) -> None:
    ...
    def _row_to_job(self, row: sqlite3.Row) -> ProcessingJob:
        ...
        if row["artifacts"]:
            try:
                artifacts_data = json.loads(row["artifacts"]) if isinstance(row["artifacts"], str) else row["artifacts"]
...
class BackgroundService:
    ...
    async def submit_job(
        ...
        **kwargs
        ...
        if existing_job and existing_job.status in [
            JobStatus.PENDING, JobStatus.PROCESSING, JobStatus.INGESTING,
            JobStatus.CHUNKING, JobStatus.EMBEDDING, JobStatus.MAPPING,
            JobStatus.GRAPH_BUILDING, JobStatus.EVALUATING, JobStatus.HIERARCHY
        ]:
            logger.info(f"Document {document_id} already has active job: {existing_job.job_id}")
    ...
    async def submit_job_with_callback(
        ...
        **kwargs
        ...
        task = asyncio.create_task(
            self._execute_job_with_callbacks(job, process_func, on_complete, on_error, **kwargs)
        )
        self._active_tasks[job.job_id] = task
        ...
        async with self._semaphore:
            try:
                ...
                if job.retry_count < job.max_retries:
                    ...
                    logger.info(f"Retrying job {job.job_id} in {delay}s (attempt {job.retry_count})")
                    
                    await asyncio.sleep(delay)
                    asyncio.create_task(
                        self._execute_job(job, process_func, **kwargs)
                    )
                else:
                    # Max retries reached
                    job.status = JobStatus.FAILED
    ...
    def check_recoverable(self, document_id: str) -> Dict[str, Any]:
        ...
        if graph_path.exists():
            ...
            available_stages.extend(["ingestion", "chunking", "embedding", "mapping", "graph_building"])
        elif settings.allow_legacy_pickle_loading:
            legacy_graph = graph_pickle_path(document_id)
            if legacy_graph.exists():
                artifacts["graph"] = {"path": str(legacy_graph), "stage": "graph_building"}
                available_stages.extend(["ingestion", "chunking", "embedding", "mapping", "graph_building"])
        
        knowledge_map_path = settings.data_dir / "knowledge_maps" / f"{document_id}.json"
        if knowledge_map_path.exists():
            artifacts["knowledge_map"] = {"path": str(knowledge_map_path), "stage": "mapping"}
    ...
    async def recover_job(
        ...
        **kwargs
        ...
        if not recovery_info["recoverable"]:
            logger.warning(f"Document {document_id} not recoverable: {recovery_info['reason']}")
            return None
        
        logger.info(f"Recovering document {document_id}, stages: {recovery_info['available_stages']}")
        ...
        task = asyncio.create_task(
            self._execute_job_recoverable(job, process_func, recovery_info["available_stages"], skip_completed_stages, **kwargs)
        )
        self._active_tasks[job.job_id] = task
        ...
        **kwargs
        ...
        async with self._semaphore:
            try:
                ...
                logger.info(f"Starting recoverable job {job.job_id} for {job.document_id}")
    ...
    def get_queue_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "current_concurrent": len([j for j in active_jobs if j.status == JobStatus.PROCESSING]),
            "queued_jobs": len([j for j in active_jobs if j.status == JobStatus.PENDING]),
